...
> **评估日期**: 2026-04-13（最终更新 2026-04-14）
> **最终结果**: **7 PASS / 0 FAIL / 1 SKIP** ✅
...
| LibreOffice | 已安装（brew install --cask libreoffice） |
| Ollama | 运行中, gemma4:e4b 已就绪 |
| OpenClaw | 2026.4.5 |
| 主模型 | ollama/gemma4:e4b |
| Gateway | 127.0.0.1:18789, pid 687, RPC ok |
...
| knowledge_base_docs_live | knowledge-base-update | 在线 | ✅ PASS | 文档转换流程成功（安装 LibreOffice 后通过） |
...
**修复**: 添加 `from __future__ import annotations`，commit `6d907b0`。
...
**解决**: 编写 `scripts/gemma_probe_driver.py`（纯 Python wrapper），通过 urllib 调用 Ollama API，commit `308ccfb`。
...
### 5. LibreOffice 未安装（已解决）✅

**问题**: `knowledge_base_docs_live` 依赖 LibreOffice 进行 PDF/DOCX 转 Markdown。
**解决**: `brew install --cask libreoffice`，安装后 knowledge_base_docs_live 通过。
...
| knowledge_base_docs_live | ⏭ SKIP | ✅ PASS |
...
| **总计** | **3P/0F/3S** | **7P/0F/1S** |

Mac 评测更全面（多覆盖了 2 个 agent probe，knowledge_base_docs_live 和 knowledge_base_rules_live 均通过）。
...
- [ ] `openclaw doctor --repair` 修复 nvm 路径警告

---

## 结论

1. **Gemma 本地模型在 Mac 上零失败**，所有可评估场景全部通过（7 PASS / 0 FAIL / 1 SKIP）
2. **Agent probe 驱动成功集成**，`gemma_probe_driver.py` 验证了 Gemma API 驱动的可行性
3. **qmd + LibreOffice 生态打通**，知识库文档流程和规则流程均验证通过
