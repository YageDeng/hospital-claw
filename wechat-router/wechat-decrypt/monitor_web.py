...
MAX_LOG = 500
_msg_store: MessageStore = None  # initialized in main()
...
def broadcast_sse(msg_data):
    ...
    if _msg_store:
        if not event_type:
            _msg_store.insert(msg_data)
        elif event_type == 'image_update':
            _msg_store.update_image(
                msg_data.get('timestamp', 0),
                msg_data.get('username', ''),
                msg_data.get('image_url', ''),
            )
        elif event_type == 'rich_update':
            _msg_store.update_rich(
                msg_data.get('timestamp', 0),
                msg_data.get('username', ''),
                msg_data.get('rich', {}),
            )
...
class SessionMonitor:
    ...
    def _resolve_display_name(self, username):
        """Resolve username to display name, trying live contact.db via db_cache as fallback."""
        display = self.contact_names.get(username)
        if display:
            return display
        ...
        contact_key = os.path.join("contact", "contact.db")
        dec_path = self.db_cache.get(contact_key)
        if not dec_path:
            return username
        try:
            ...
            conn.close()
            if row:
                name = row[1] if row[1] else row[0] if row[0] else username
                self.contact_names[username] = name
                if _msg_store and name != username:
                    _msg_store.backfill_chat_name(username, name)
                return name
        except Exception as e:
            print(f"  [contact] 查询 contact.db 失败: {e}", flush=True)

        return username

    # WeChat base_type → attach subdirectory mapping
    _MEDIA_SUBDIRS = {3: 'Img', 43: 'Video', 34: 'Voice'}
    _MEDIA_LABELS = {3: 'img', 43: 'video', 34: 'voice'}
    _MEDIA_CACHE_EXTS = {
        3: ('jpg', 'png', 'gif', 'webp'),
        43: ('mp4', 'avi', 'mov', 'mkv', 'jpg', 'png'),
        34: ('silk', 'amr', 'mp3'),
    }

    def resolve_media(self, username, timestamp, base_type=3):
        """Decrypt media: username+timestamp+base_type → decoded filename, or None.

        Works for images (3), videos (43), and voice (34).
        Returns '__v2_unsupported__' if V2 encryption without key.
        """
        if not self.db_cache or not self.username_db_map:
            return None

        label = self._MEDIA_LABELS.get(base_type, 'media')
            ...
            for _try in range(2):
                ...
                try:
                    ...
                    row = conn.execute(f"""
                        ...
                        WHERE (local_type = {base_type} OR (local_type > 4294967296 AND local_type % 4294967296 = {base_type}))
                        AND create_time = ?
                    """, (timestamp,)).fetchone()
                    if not row:
                        row = conn.execute(f"""
                    ...
                    if 'malformed' in str(e) and _try == 0:
                        print(f"  [{label}] {db_key} malformed, 强制刷新...", flush=True)
                        self.db_cache.invalidate(db_key)
                        continue
                    if 'no such table' not in str(e):
                        print(f"  [{label}] 查询 {db_key}/{table_name} 失败: {e}", flush=True)
                    break
            if local_id:
                break

        if not local_id:
            print(f"  [{label}] 未找到 local_id: {username} t={timestamp}", flush=True)
            return None

        res_key = os.path.join("message", "message_resource.db")
        file_md5 = None
        for _try in range(4):
            # On retries, invalidate cache so we re-decrypt with latest WAL data
            if _try > 0:
            ...
            try:
                ...
                row = conn.execute(
                    ...
                    f"AND (message_local_type = {base_type} OR message_local_type % 4294967296 = {base_type})",
                    (local_id, timestamp)
                ).fetchone()
                if not row:
                    row = conn.execute(
                ...
                if row and row[0]:
                    ...
                    break
                if _try < 3:
                    print(f"  [{label}] MD5未就绪, 重试({_try+1})...", flush=True)
            except Exception as e:
                if 'malformed' in str(e) and _try < 3:
                    print(f"  [{label}] resource DB malformed, 强制刷新...", flush=True)
                    continue
                print(f"  [{label}] 查询 message_resource 失败: {e}", flush=True)
                return None

        if not file_md5:
            print(f"  [{label}] 未找到 MD5: local_id={local_id} t={timestamp}", flush=True)
            return None
        ...
        if not os.path.isdir(search_base):
            print(f"  [{label}] attach 目录不存在: {search_base}", flush=True)
            return None

        # Search in the type-specific subdir first, then try all subdirs
        primary_subdir = self._MEDIA_SUBDIRS.get(base_type, 'Img')
        dat_files = sorted(glob_mod.glob(
            os.path.join(search_base, "*", primary_subdir, f"{file_md5}*.dat")))
        if not dat_files:
            dat_files = sorted(glob_mod.glob(
                os.path.join(search_base, "*", "*", f"{file_md5}*.dat")))
        if not dat_files:
            print(f"  [{label}] 未找到 .dat: MD5={file_md5}", flush=True)
            return None
        ...
        cache_exts = self._MEDIA_CACHE_EXTS.get(base_type, ('jpg', 'png', 'gif', 'webp', 'mp4'))
        for ext in cache_exts:
            ...
            if os.path.exists(candidate):
                ...
                os.unlink(candidate)
                print(f"  [{label}] 缓存过小: {cached_sz/1024:.0f}KB → 重解密", flush=True)
                break

        for rank, sz, selected in ranked:
            sel_type = rank_names.get(rank, '?')
            print(f"  [{label}] 尝试 {sel_type}({sz/1024:.0f}KB): {os.path.basename(selected)}", flush=True)

            if is_v2_format(selected) and not IMAGE_AES_KEY:
                print(f"  [{label}] V2 格式缺少 AES key, 跳过", flush=True)
                continue

            result_path, fmt = decrypt_dat_file(selected, f"{out_base}.tmp", IMAGE_AES_KEY, IMAGE_XOR_KEY)
            if not result_path:
                print(f"  [{label}] 解密失败, 跳过", flush=True)
                continue

            if fmt in ('hevc', 'bin') and base_type == 3:
                jpg_path = _convert_hevc_to_jpeg(result_path, f"{out_base}.jpg")
                os.unlink(result_path)
                if jpg_path:
                    size_kb = os.path.getsize(jpg_path) / 1024
                    print(f"  [{label}] HEVC→JPEG 成功: {os.path.basename(jpg_path)} ({size_kb:.0f}KB)", flush=True)
                    return os.path.basename(jpg_path)
                print(f"  [{label}] HEVC→JPEG 转换失败, 尝试下一个", flush=True)
                continue
            ...
            print(f"  [{label}] 解密成功: {os.path.basename(final)} ({size_kb:.0f}KB)", flush=True)
            return os.path.basename(final)

        print(f"  [{label}] 所有 .dat 均无法解密", flush=True)
        return '__v2_unsupported__'

    def resolve_image(self, username, timestamp):
        """解密图片: username+timestamp → 解密后的图片文件名，失败返回 None"""
        return self.resolve_media(username, timestamp, base_type=3)

    def resolve_video(self, username, timestamp):
        """解密视频: username+timestamp → 解密后的视频文件名，失败返回 None"""
        return self.resolve_media(username, timestamp, base_type=43)

    def _record_attachment(self, username, timestamp, media_type, file_name):
        """Record a resolved media file in the attachments table."""
        if not _msg_store or not file_name:
            return
        file_path = os.path.join(DECODED_IMAGE_DIR, file_name)
        file_size = os.path.getsize(file_path) if os.path.exists(file_path) else None
        md5 = file_name.split('.')[0] if '.' in file_name else None
        _msg_store.insert_attachment(
            timestamp=timestamp, username=username, media_type=media_type,
        ...
        for attempt in range(3):
            try:
                ...
                if img_name == '__v2_unsupported__':
                    ...
                    self._record_attachment(username, timestamp, 'image', img_name)
    ...
    def _async_resolve_video(self, username, timestamp, msg_data):
        """后台线程: 解密视频并通过 SSE 推送更新"""
        delays = [0.5, 1.5, 3.0]
        for attempt in range(3):
            try:
                time.sleep(delays[attempt])
                vid_name = self.resolve_video(username, timestamp)
                if vid_name and vid_name != '__v2_unsupported__':
                    media_url = f'/img/{vid_name}'
                    ...
                    broadcast_sse({
                        ...
                        'rich': {
                            'type': 'video',
                            'media_url': media_url,
                            'duration': msg_data.get('rich', {}).get('duration', 0) if isinstance(msg_data.get('rich'), dict) else 0,
                        },
                    })
                    self._record_attachment(username, timestamp, 'video', vid_name)
                    print(f"  [video] 异步解密成功: {vid_name}", flush=True)
                    return
            except Exception as e:
                print(f"  [video] 异步解密失败(attempt={attempt}): {e}", flush=True)
        ...
        for ts, base, mc in hidden_msgs:
            ...
            if base == 3:
                ...
                if img_name and img_name != '__v2_unsupported__':
                    ...
                    self._record_attachment(username, ts, 'image', img_name)
        ...
        for attempt in range(3):
            try:
                ...
                if info:
                    ...
                    # Record emoji downloads as attachments
                    if info.get('type') == 'emoji' and info.get('emoji_url', '').startswith('/img/'):
                        emoji_file = info['emoji_url'].split('/img/')[-1]
                        self._record_attachment(username, timestamp, 'emoji', emoji_file)
...
_active_monitor = None  # reference to the running SessionMonitor for post-warmup updates
...
/* 搜索 */
.search-box{display:flex;align-items:center;gap:4px;margin-left:12px}
.search-box input{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:6px;padding:5px 10px;color:#ccc;font-size:12px;width:180px;outline:none;transition:border .2s}
.search-box input:focus{border-color:rgba(79,195,247,.5);width:240px}
.search-box button{background:none;border:none;color:#888;font-size:14px;cursor:pointer;padding:2px 6px}
.search-box button:hover{color:#ccc}
.search-banner{background:rgba(79,195,247,.1);border-bottom:1px solid rgba(79,195,247,.2);padding:8px 24px;font-size:13px;color:#4fc3f7;display:flex;align-items:center;gap:12px;flex-shrink:0}
.search-banner button{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);color:#ccc;font-size:12px;padding:4px 12px;border-radius:4px;cursor:pointer}
.search-banner button:hover{background:rgba(255,255,255,.15)}
.load-more-btn{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#888;font-size:12px;padding:6px 20px;border-radius:6px;cursor:pointer;transition:all .2s}
.load-more-btn:hover{background:rgba(255,255,255,.12);color:#ccc}
#dbTotal{font-size:11px;color:#555;margin-left:4px}
</style>
</head>
<body>
<div class="header">
...
<div class="search-box"><input type="text" id="searchInput" placeholder="搜索消息..." /><button onclick="doSearch()">🔍</button><button id="searchClear" style="display:none" onclick="clearSearch()">✕</button></div>
<div class="stats"><span id="cnt">0 消息</span><span id="dbTotal" title="数据库总消息数"></span><span id="perf"></span></div>
...
<div id="searchBanner" class="search-banner" style="display:none"><span id="searchLabel"></span><button onclick="clearSearch()">返回实时</button></div>
<div class="messages" id="msgs">
<div class="empty" id="empty"><div class="icon">📡</div><p>等待新消息...</p><p style="margin-top:6px;font-size:11px;color:#333">WAL增量解密 · SSE推送 · SQLite存储</p></div>
</div>
<div id="loadMoreWrap" style="display:none;text-align:center;padding:8px"><button class="load-more-btn" onclick="loadMore()">加载更多历史消息</button></div>
<script>
let n=0;
...
function addMsg(m, animate, appendToBottom){
...
function doSearch(){
  ...
  document.getElementById('searchLabel').textContent = '搜索: "' + q + '"';
  document.getElementById('searchClear').style.display = '';
  M.innerHTML = '';
  seen.clear();
  n = 0;
  fetch('/api/search?q=' + encodeURIComponent(q) + '&limit=100').then(r=>r.json()).then(ms=>{
    if(!ms.length){ M.innerHTML = '<div class="empty"><div class="icon">🔍</div><p>未找到匹配消息</p></div>'; return; }
    ms.sort((a,b)=>a.timestamp-b.timestamp);
    ms.forEach(m=>addMsg(m, false));
    document.getElementById('loadMoreWrap').style.display = ms.length >= 100 ? '' : 'none';
  });
}
...
function loadMore(){
  if(searchMode){
    const q = document.getElementById('searchInput').value.trim();
    historyOffset += 100;
    fetch('/api/search?q=' + encodeURIComponent(q) + '&limit=100&offset=' + historyOffset).then(r=>r.json()).then(ms=>{
      ms.sort((a,b)=>b.timestamp-a.timestamp);
      ms.forEach(m=>addMsg(m, false, true));
      if(ms.length < 100) document.getElementById('loadMoreWrap').style.display = 'none';
    });
  } else {
    historyOffset += HISTORY_PAGE;
    fetch('/api/history?limit=' + HISTORY_PAGE + '&offset=' + historyOffset).then(r=>r.json()).then(ms=>{
      ms.sort((a,b)=>b.timestamp-a.timestamp);
      ms.forEach(m=>addMsg(m, false, true));
      if(ms.length < HISTORY_PAGE) document.getElementById('loadMoreWrap').style.display = 'none';
    });
  }
}
document.getElementById('searchInput').addEventListener('keydown', e=>{ if(e.key==='Enter') doSearch(); });
document.getElementById('searchInput').addEventListener('input', e=>{ if(!e.target.value) clearSearch(); });

function loadInitialHistory(){
  fetch('/api/history?limit=' + HISTORY_PAGE).then(r=>r.json()).then(ms=>{
    ms.sort((a,b)=>a.timestamp-b.timestamp);
    ms.forEach(m=>addMsg(m, false));
    document.getElementById('loadMoreWrap').style.display = ms.length >= HISTORY_PAGE ? '' : 'none';
    connectSSE();
  });
}
...
fetch('/api/stats').then(r=>r.json()).then(s=>{
  let info = '';
  if(s.total_messages) info += s.total_messages + '消息';
  if(s.total_attachments) info += ' / ' + s.total_attachments + '附件';
  if(info) document.getElementById('dbTotal').textContent = '(DB: ' + info + ')';
});
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ('/', '/index.html'):
            ...
            if _msg_store and (offset > 0 or username or chat or start or end):
                data = _msg_store.get_history(
                    limit=min(limit, 500), offset=offset,
                    username=username, chat_name=chat,
                    start_time=start_ts, end_time=end_ts,
                )
            elif _msg_store:
                data = _msg_store.get_history(limit=min(limit, 500))
            else:
                with messages_lock:
            ...
            if not keyword or not _msg_store:
                data = []
            else:
                data = _msg_store.search(keyword, limit=min(limit, 200), offset=offset, chat_name=chat)
            ...
            stats = _msg_store.get_stats() if _msg_store else {'total_messages': 0}
            self.send_response(200)
            self.send_header('Content-Type', 'application/json; charset=utf-8')
            self.end_headers()
            ...
            parsed = urllib.parse.urlparse(self.path)
            qs = urllib.parse.parse_qs(parsed.query)
            msg_id = qs.get('message_id', [None])[0]
...
def main():
    global _msg_store, messages_log

    print("=" * 60, flush=True)
    print("  微信实时监听 (WAL增量 + SSE推送 + SQLite存储)", flush=True)
    print("=" * 60, flush=True)

    # Initialize persistent message store
    msg_db_path = _cfg.get("message_db", "messages.db")
    if not os.path.isabs(msg_db_path):
        msg_db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), msg_db_path)
    _msg_store = MessageStore(msg_db_path)
    print(f"消息存储: {msg_db_path}", flush=True)

    # Pre-load recent messages from DB into in-memory log for SSE replay
    stored = _msg_store.get_recent(MAX_LOG)
    if stored:
        with messages_lock:
            messages_log = stored
        print(f"从数据库加载 {len(stored)} 条历史消息", flush=True)
    ...
    # Backfill any raw usernames stored as chat_name in previous runs
    if _msg_store and contact_names:
        for username, display in contact_names.items():
            if username != display:
                _msg_store.backfill_chat_name(username, display)
    ...
    def _warmup():
        try:
            t0 = time.perf_counter()
            # Decrypt contact.db first so names are available
            contact_key = os.path.join("contact", "contact.db")
            if get_key_info(keys, contact_key):
                t1 = time.perf_counter()
                try:
                    db_cache.get(contact_key)
                    print(f"[warmup] {contact_key} {(time.perf_counter()-t1)*1000:.0f}ms", flush=True)
                except Exception as e:
                    print(f"[warmup] {contact_key} 失败: {e}", flush=True)
        ...
        # Rebuild contact names from the db_cache decrypted copy (not the static decrypted/ dir)
        fresh_contacts = {}
        dec_contact = db_cache.get(os.path.join("contact", "contact.db"))
        if dec_contact:
            try:
                conn = sqlite3.connect(f"file:{dec_contact}?mode=ro", uri=True)
                for r in conn.execute("SELECT username, nick_name, remark FROM contact").fetchall():
                    fresh_contacts[r[0]] = r[2] if r[2] else r[1] if r[1] else r[0]
                conn.close()
            except Exception as e:
                print(f"[warmup] 联系人加载失败: {e}", flush=True)

        # Rebuild username→DB map from the db_cache decrypted message DBs
        fresh_map = {}
        for i in range(5):
            rel_key = os.path.join("message", f"message_{i}.db")
            dec_path = db_cache.get(rel_key)
            if not dec_path:
                continue
            try:
                conn = sqlite3.connect(f"file:{dec_path}?mode=ro", uri=True)
                for row in conn.execute("SELECT user_name FROM Name2Id").fetchall():
                    if row[0]:
                        if row[0] not in fresh_map:
                            fresh_map[row[0]] = []
                        fresh_map[row[0]].append(rel_key)
                conn.close()
            except Exception as e:
                print(f"[warmup] Name2Id message_{i}.db: {e}", flush=True)

        print(f"[warmup] 重新加载联系人: {len(fresh_contacts)} 个", flush=True)
        print(f"[warmup] 重新映射用户名: {len(fresh_map)} 个", flush=True)

        # Push updated maps into the running SessionMonitor
        if _active_monitor:
            _active_monitor.contact_names.update(fresh_contacts)
            _active_monitor.username_db_map = fresh_map
            print("[warmup] 已更新 SessionMonitor 联系人和DB映射", flush=True)

        # Backfill chat_name in message store
        if _msg_store and fresh_contacts:
            for uname, display in fresh_contacts.items():
                if uname != display:
                    _msg_store.backfill_chat_name(uname, display)
